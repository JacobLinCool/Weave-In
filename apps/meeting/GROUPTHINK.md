...
All four actions prepare a short question silently. The persistent Omni card in Room shows an analysis status during preparation. Once the question is ready, the Allow Omni to speak button glows; click the glowing button to approve. After participant approval, Omni reads only that question aloud to everyone and records it in the public Transcript. The question is not posted as a Room chat message. These actions do not modify a whiteboard or operate other users' controls. The kind-to-action mapping is fixed by the application policy.

## Coordination and safeguards

A hidden runner or a runner without working speech monitoring yields Omni to another ready foreground browser, because voice monitoring depends on visible-frame sampling. Speech already being published keeps its valid turn until completion or expiry. This does not revoke personal Muse audio. If no suitable browser remains, Omni waits until one becomes available. Returning to the tab requires a new quiet interval; fresh discussion received while unavailable is still eligible, while historical replay alone is not. If audio monitoring cannot initialize, that device shows an error.
