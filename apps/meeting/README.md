...
3. In **Muse → Settings**, confirm **Room messages** is disabled. Close settings, choose **Talk**, and ask, “Use our discussion and the uploaded reference to draw the workflow on the whiteboard. Keep your reply private.”
4. Check that the whiteboard opens and both participants see the diagram. Muse's spoken response should stay private, and no assistant message should appear in Room.
5. Choose **Finish speaking** to end the private interaction and restore the meeting microphone to its previous state.

To test public posting separately, create an assistant with **Allow posting to Room (visible to everyone)** checked, then explicitly ask “Post the agreed steps in Room chat.” Verify that both participants see the message with the owner's agent attribution. Changing an existing assistant requires removing it first, which clears this tab's private Muse conversation.
