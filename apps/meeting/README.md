...
`TYPESAFE_API_KEY` is required for Omni detection; `OPENAI_API_KEY` is required for Muse and Omni question drafting/speech, even when captions and automatic reminders use Gemini. Failed or uncertain Jev requests do not start a drafting session or switch detectors. Voice model constants are in `src/agents/contracts.ts`; detection settings are in `src/agents/group-detection.ts` and `src/agents/group.ts`. Browser audio activation may be needed for Muse’s spoken replies; creating the personal configuration on join does not itself open a continuous Live connection. The settings button in Muse edits source and tool permissions in place.
...
# Omni meeting detection:
pnpm --filter @weave-in/meeting exec wrangler secret put TYPESAFE_API_KEY
# Muse, Omni drafting/speech, and optional OpenAI captions:
...
The browser harness creates two independent browser contexts with real room WebSockets and peer connections, using simulated Jev detections and a local WebRTC provider simulator for GPT-Live initialization and mocked TURN provisioning for local peer connectivity. It verifies automatic Muse setup without a Live connection, private isolation, approved-only public speech with owner attribution, an open owner microphone and interruption without resume, silent Omni preparation followed by explicitly approved public audio and transcript, private conversation recovery, signaling reconnection, public replay/deduplication, and mobile layout. [VERIFICATION.md](VERIFICATION.md) separates current harness coverage from historical browser/provider results. The same harness accepts a third options argument `{ groupOnly: true, scenario: "convergence" | "drift" | "float" | "echo" | "none", realProvider: true }` to exercise automatic Group reviews against real Jev detection and GPT-Live drafting/speech. Omit `realProvider` for deterministic provider simulation. It sends normal Room chat through the UI; it never injects review commands. Each scenario uses a separate room. Intervention scenarios wait for a raised draft, assert that no public output occurs before approval, then approve it and check peer delivery and audio. The `none` scenario checks abstention. Provider simulation checks integration; real-provider runs additionally sample Jev classification and read-aloud behavior. Run `node apps/meeting/scripts/evaluate-group-detection.mjs` from the repository root for a focused real-Jev evaluation using synthetic English and Traditional Chinese meetings; it reads the local TypeSafe key, reports actual confidence, latency and abstentions, and exits nonzero on a wrong semantic classification or confidently misrouted intervention. A correct classification below the confidence threshold is reported as an abstention. Human listening and noisy-room evaluation remain separate.
