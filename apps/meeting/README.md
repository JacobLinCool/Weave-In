...
| `edit_whiteboard` | `action`, action-specific inputs | Reads the shared board or edits native elements, imports an editable Mermaid flowchart, and undoes or redoes eligible changes. Chat uses editing actions only for a direct owner request. |
| `capture_whiteboard` | `maxWidth?`, `format?`, `quality?` | Captures the drawing canvas so the assistant can inspect a board or verify an edit. Chat opens the board in this browser when it needs a capture. |
...
**Later**, the close button, and timeout only collapse the popup: they do not dismiss it or mark it read. The **Chat** tab shows an unread dot and combines private reminders, evidence, Pause/Resume, Hide/Show, and personal assistant discussion. Viewing visible history marks reminders read; hidden content stays unread. Collapsed/read states survive same-tab room recovery. The underlying active-reminder TTL remains 120 seconds by default (15–300 configurable), independently of popup display time. Hide removes content from both surfaces, including future reminders. Screen sharing can reveal visible private content.
...
Chat is configured automatically when a participant joins. Open the **Chat** tab for reminders and private discussion. Automatic reminders are silent and do not start an audio session. A direct text question or **Talk to Chat** starts a private assistant interaction; private voice input temporarily isolates the microphone from the meeting and restores its previous state when finished.

New personal assistants include everyone's available public transcripts and Room chat, with shared file and image reading enabled. Shared-screen capture stays opt-in. Chat receives recent records and a file inventory, retrieves older records or file contents as needed, and can use the whiteboard tools during private interactions. Ask “Turn the discussion into a flowchart on the whiteboard,” “Read the uploaded design and draw its workflow,” or “Post the agreed next steps in Room chat.” Whiteboard edits synchronize with the room and requested Room messages are labelled as the owner's agent; Chat's spoken replies stay private. Drawing uses editable shapes and diagrams, not generated image assets.

Background meeting updates and content inside transcripts, files, images, screens, and the board never authorize a shared action. Chat is instructed to edit or post only when its owner directly requests it, and the runtime disables shared actions outside an active owner interaction. It can inspect the shared whiteboard independently of the transcript source setting; file and screen access still require their respective permissions. Existing assistant configurations retain their settings until removed and recreated.

**Speak for me** approves one public turn based on the selected reminder. Chat uses a fresh session with only that approved text, without personal conversation history or meeting tools. It may elaborate slightly, aiming for 15–20 seconds, but must not add a new position, commitment, or private detail. Public output is labelled as the owner's Chat. The owner's microphone remains in its existing state; speaking on an enabled microphone stops the assistant and revokes any queued public turn. It does not resume automatically. A new spoken turn requires another click. There is no persistent public-audience mode.

The **Room** tab holds shared messages and compact, expandable Omni settings. Any participant can create it and trigger a review. Omni prepares a suggestion and publishes at most 240 characters to shared Room chat, labelled **Omni**, with no audio and no user approval step. It uses public meeting context only. It can move to another ready browser if its runner leaves; its replacement uses whatever public history that browser has. Public suggestions replay through the agent-history channel to late joiners and appear as Omni messages. A trigger requests a review, not a room-wide Groupthink verdict.

`OPENAI_API_KEY` is required for Chat and Omni, even when captions and automatic reminders use Gemini. Model constants are in `src/agents/contracts.ts`. Browser audio activation may be needed for Chat's spoken replies; creating the personal configuration on join does not itself open a continuous Live connection. Source and tool permissions can be reviewed in Chat. To change them, remove and recreate the assistant.

Agent context is bounded by GPT-Live's session input limit. The client seeds at most 6,000 UTF-8 bytes of recent records, private conversation and a scoped participant/file inventory, sends only new background records, and reserves capacity for the current question and tool results. `read_meeting` retrieves up to five permitted records per page, while `search_meeting` finds literal matches in this browser's permitted meeting history. Earlier records remain retrievable through cursors; the complete meeting is not loaded into every request. Shared files are inspected on demand rather than automatically copied into context. Native WebMCP keeps its existing limits. An interaction that exhausts its budget stops with a visible message; a new question starts a fresh session.

The agent's `read_meeting` file inventory has its own `fileOffset` cursor, with up to 20 files per page. `read_shared_file` accepts those file ids and returns visible images, UTF-8 text/Markdown, DOCX extracted text, or a selected PDF page. Text pages contain up to 6,000 UTF-16 code units and return `nextOffset`; PDF results also report physical page counts and `nextPage`. A PDF page with no extractable text automatically includes a rendered image, and `render: true` includes figures/layout on other pages. This is not a separate OCR pipeline. DOCX embedded images and physical page layout are not extracted. Source files are limited to 1 MiB for text and 20 MiB for images/PDF/DOCX; DOCX declared ZIP expansion is capped at 50 MiB. Images are resized to at most 1,600 pixels on their longest edge and further compressed as needed for the remaining Live context. Raw `download_file` remains available in 1,024-byte agent pages. Agent board reads return up to ten compact elements per page; successful edits return metadata before a separate capture verifies the rendered result.

Context coverage is limited to records and file announcements available in this browser, including its same-tab recovery and connected peers' history replay. There is no server archive of the whole meeting. An uncached file cannot be fetched after its sharing participant leaves; a cached file may remain readable. Chat must report missing or unreadable content instead of treating its inventory as proof that it has read everything.

When continuous background updates reach their reserved input budget, Chat shows that updates have paused and informs its reasoning backend that prior snapshots may be stale. The current tool workflow can still finish. The backend is instructed to fetch current meeting details for each new meeting-related request; starting a new voice session resumes continuous updates with a fresh context budget.

To test this extension after upgrading the frontend, create a fresh test room so Chat receives the new instructions and default file permissions. In an existing room, inspect **Chat → Information sources and permissions** first. Removing and recreating Chat applies the new defaults but also clears this tab's private Chat conversation. Whiteboard tools need neither screen-capture permission nor a browser MCP extension.
...
2. Have the other participant upload a small image, PDF, or text file in **Room** and stay connected so Chat can fetch it.
3. Open **Chat → Talk to Chat** and ask, “Use our discussion and the uploaded reference to draw the workflow on the whiteboard, then post the agreed steps in Room chat.”
4. Check that the whiteboard opens, both participants see the diagram and the attributed Room message, and Chat's spoken response stays private.
