async (page, origin = 'http://127.0.0.1:8788') => {
  ...
  try {
    ...
    await page.locator('.agent-panel--personal').getByRole('button', { name: 'Settings', exact: true }).click();
    await page.getByRole('dialog', { name: 'Muse settings', exact: true }).getByText('Disabled — private replies only', { exact: true }).waitFor();
