...
export function scopedTools(base: MeetingToolsContext, config: AgentConfig, owner: string, active: () => boolean, canPublish: () => boolean): ToolDefinition[] {
  ...
  return tools.filter((tool) =>
    ...
    ...(tool.name === 'edit_whiteboard' ? { description: 'Read or edit the shared whiteboard. Choose action read to inspect object ids and coordinates: pages contain up to 10 compact records; continue with nextOffset. For a new structured node-and-connection diagram, choose action mermaid with flowchart/graph syntax (including subgraphs): this adds native editable objects, preserving existing content. Source is limited to 12000 characters and imports to 300 elements; other Mermaid diagram families and image fallbacks are unsupported. Choose action edit for precise positioning, free-form layouts or changes to existing objects, using up to 50 create/update/delete operations. Supported kinds are note, rectangle, diamond, text, pen and connector; text is limited to 500 characters per node. Use unique create ids and from/to ids for connected nodes. Deleting a node removes its connectors. Undo/redo affects this participant’s latest undoable edit, including manual edits. Mutations require the current owner’s explicit request. After editing, use capture_whiteboard to verify the visible result.',
    ...
    ...(tool.name === 'send_chat_message' ? { description: `${tool.description} For this assistant, send only text: the application always attributes the post to the configured assistant name, regardless of any agent argument. Use only for an explicit request to publish this content to everyone in Room.` } : {}),
    execute: async (input) => {
