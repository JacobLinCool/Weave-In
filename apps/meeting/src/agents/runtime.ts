  ...
  async #start(agent: RoomAgent, audience: Audience, preparing: boolean, text: string, voice: boolean, inputLine?: AgentLine): Promise<void> {
    ...
    try {
      ...
      const live = new AgentLive({
        ...
        contextPaused: () => {
          if (!valid()) return;
          this.#status = 'Background updates paused to reserve space for tools. Chat can fetch current meeting details when asked. Start a new voice session for continuous updates.';
          this.#emit();
        },
        error: (message) => {
