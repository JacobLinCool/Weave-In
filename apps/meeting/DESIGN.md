...
Private reminders share a wrapping footer with the meeting controls, without overlaying captions. Empty or hidden reminders collapse to a compact header with automatic monitoring status and Pause/Resume; an active reminder expands to fit its text, with scrolling for longer content. On narrow screens the footer stacks. Cotton text and a lock label identify private content; no participant dye, sound, autofocus, or entrance animation is used. A polite screen-reader status announces the current reminder. The Private side-panel tab holds evidence and history. Hide removes content from both surfaces; it is available even when there are no reminders. Follow-up discussion belongs in the existing assistant session.
