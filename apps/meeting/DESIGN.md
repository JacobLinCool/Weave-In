...
Muse is configured for each participant on join, without starting a spoken session. Reminder actions offer **Discuss privately** and **Speak for me**; closing or timing out never grants permission. Completed private Muse replies also offer a **Send** icon vertically centered on the right when hovered or keyboard-focused, with a subtle Raised background. Touch users tap the message to reveal it. Send and Speak for me approve only the selected text for one faithful public reading, without elaboration or private context. A visible Stop control cancels the turn, with a ten-minute safety timeout for stalled sessions. The owner's microphone stays in its existing state; owner speech interrupts Muse with no automatic resume. Shared transcript labels identify whose Muse spoke.
...
## Landing composition: agent-native shared thinking

The current landing extends the Weaver’s Draft identity with the existing indigo surfaces, cotton type, weld gold, human thread dyes, Jost headings, and thin ruled sheets. It introduces no new design tokens. The English narrative moves from lost ideas and critical thinking through Omni, Muse, and Codex; agent access through WebMCP; the structured shared whiteboard; a contribution workflow; privacy; and the closing create-room action. These named capabilities are presented as available product behavior.

The desktop hero pairs the headline and labelled synthetic woven conversation with the room panel. Subsequent sections use an editorial heading-and-content split, open ruled agent rows, a compact context flow, and a numbered workflow. The whiteboard explains shared structure through a responsive SVG payment diagram: native buttons labelled “Codex draws” and “You refine” expose their selected state with `aria-pressed`, switching the retry node, accessible image description, and caption. The figure is explicitly illustrative and shows how a human edit remains readable by Codex.

At 1080px and below, the hero and content sheets collapse to one column; invited visitors see the room panel before the woven draft. At 720px and below, navigation, typography, thread geometry, and board-control spacing become more compact. Landing controls retain visible keyboard focus. Camera and microphone preview, settings, display name, room-code entry, invite prefill, create/join callbacks, and loading/error states keep their existing behavior. Privacy remains a three-part account of participant WebRTC/TURN transport, AI-provider access, and Worker processing.
